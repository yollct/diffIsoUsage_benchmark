...
resgene <- do.call(c, lapply(allfolders, function(thisfolder){
    ...
    files <- Sys.glob(sprintf("/nfs/home/students/chit/is_benchmark/singlecells/rawdata/simulation_scd/unbalance%s/*/quant.sf", r))
    names(files) <- gsub(".*/","",gsub("/quant.sf","",files))
    cts <- import_counts(files = files, type = "salmon")

    ### dropout ####
    dropout_events <- matrix(
            rbinom(ncol(cts) * nrow(cts), 1, 1 - dr),
            nrow = nrow(cts), ncol = ncol(cts)
        )

    # Apply dropout
    dropout_cts <- cts * dropout_events
    # cts <- combine_to_matrix(tx_list = cts)

    pd <- data.frame("sample_id"=colnames(dropout_cts), "group"=factor(metadata$group, levels=unique(metadata$group)), 
    ...
    dturtle_g <- dturtle_g %>% reframe(fdr = min(fdr), tool=tool, dropout=dr, .by="feature_id") %>% unique
    ...
    saturn_g <- do.call(rbind, lapply(allcomb, function(x){
        ...
        thiscts <- dropout_cts[filter_edgeR,]
        ...
        saturn_g <- data.frame(feature_id=res$gene_id, fdr=res$regular_FDR, tool="satuRn", dropout=dr, comb=x)
    }))

    saturn_g <- saturn_g %>% reframe(fdr = min(fdr), tool=tool, dropout=dr, .by="feature_id") %>% unique
    print("done")
...
truepos <- lapply(allfolders, function(thisfolder){
    ...
    print("run")
    print(r)
    new_geneinfo <- read.csv(sprintf("/nfs/home/students/chit/is_benchmark/singlecells/rawdata/simulation_scd/%s/sim_unbalance_2ct_rep%s_geneinfo.csv", thisfolder, r), sep="\t")
...
final_long <- read.csv("/nfs/proj/is_benchmark/singlecells/rawdata/simulation_scd/singlecell_dropout_unbalance_finallong.csv", sep=" ")
png("singlecell_dropout_unbalance_prere.png", res=300, width=3000, height=1000)
