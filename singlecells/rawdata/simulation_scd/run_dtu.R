...
resgene <- do.call(c, lapply(allfolders, function(thisfolder){
    ...
    files <- Sys.glob(sprintf("/nfs/proj/is_benchmark/singlecells/rawdata/simulation_scd/balance%s/*/quant.sf", r))
    names(files) <- gsub(".*/","",gsub("/quant.sf","",files))
    cts <- import_counts(files = files, type = "salmon")

    # cts <- combine_to_matrix(tx_list = cts)

    pd <- data.frame("sample_id"=colnames(cts), "group"=c(rep("celltype1", r), rep("celltype2", r)), 
    ...
    dturtle_g <- data.frame(feature_id=dturtle_1$FDR_table$geneID, fdr=dturtle_1$FDR_table$gene, tool="DTUrtle", rep=r) %>% unique
    dturtle_tx <- data.frame(feature_id=dturtle_1$FDR_table$txID, fdr=dturtle_1$FDR_table$transcript, tool="DTUrtle", rep=r)
    ...
    tx2gene <-  tx2gene[which(tx2gene$transcript_id %in% row.names(cts)),]
    tx2gene <- subset(tx2gene, 
                 duplicated(tx2gene$gene_name) | duplicated(tx2gene$gene_name, fromLast = TRUE))
    cts <- cts[which(row.names(cts) %in% tx2gene$transcript_id), ]
    tx2gene <- tx2gene[match(row.names(cts), tx2gene$transcript_id),]
...
testpos <- unlist(testpos, recursive=F)
names(testpos) <- Map(paste0, base::rep(allfolders, each=length(reps)*2), tools, base::rep(reps, each=2), sep="")
...
write.table(final_long, "singlecell_balance_finallong.csv")
