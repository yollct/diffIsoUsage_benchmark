...
def parse_args():
    parser = argparse.ArgumentParser("Read salmon")
...
if __name__=="__main__":
    ...
    for x in os.listdir(args.dir):
        ...
        if args.pattern in x:
            tmp = pd.read_csv(os.path.join(args.dir, x, "quant.sf"), sep="\t")
            if args.type=='TPM':
                reads.append(pd.Series(tmp['TPM'], name=x))
            else:
                reads.append(pd.Series(tmp['NumReads'], name=x))
            #reads.append(pd.Series(tmp['TPM'],name=x)) 
    ...
    isodf.to_csv(os.path.join(args.outputfile, "raw_salmon_isoktsp_count.txt"), sep="\t", index=False)
    finaldf['gene_id'] = gene_id
    finaldf['feature_id'] = tmp['Name']
    finaldf.to_csv(os.path.join(args.outputfile, "salmon_count.csv"), index=False)

    f=open(os.path.join(args.outputfile, "raw_salmon_isoktsp_count.txt"), "r")
    firstline, rest = f.readline(), f.read()
    t=open(os.path.join(args.outputfile, "salmon_isoktsp_count.txt"), "w")
    t.write(firstline[1:])
    t.write(rest)
    t.close()
    f.close()
