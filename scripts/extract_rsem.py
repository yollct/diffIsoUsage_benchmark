...
def parse_args():
    parser = argparse.ArgumentParser("Read rsem")
...
if __name__=="__main__":
    ...
    for x in os.listdir(args.dir):
        ...
        tmp = pd.read_csv(os.path.join(args.dir,x, f"{x}.isoforms.results"), sep="\t")
        if args.type=='TPM':
            reads.append(pd.Series(tmp['TPM'], name=x))
        else:
            reads.append(pd.Series(tmp['expected_count'], name=x))
            #reads.append(pd.Series(tmp['TPM'],name=x)) 
    #reads.append(pd.Series(tmp['transcript_id'], name="feature_id"))
    ...
    # if "." in tmp['Name'][0]:
    #     genemapper = dict(zip(gtf['transcript_id']+"."+gtf['transcript_version'], gtf['gene_id']))
    # else:
    ...
    isodf['isoform_id'] = tmp['transcript_id']
    ...
    #     except:
    #         return "?"
    # gene_id = list(map(lambda x: mapping(x), tmp['Name']))

    isodf['isoform_id'] = pd.Series(gene_id) + "," + isodf['isoform_id']
    isodf.columns = [""] + meta['name_isoktsp'].to_list() 

    isodf.to_csv(os.path.join(args.outputfile, "raw_rsem_isoktsp_count.txt"), sep="\t", index=False)
    finaldf['gene_id'] = gene_id
    finaldf.to_csv(os.path.join(args.outputfile, "rsem_count.csv"), index=False)

    f=open(os.path.join(args.outputfile, "raw_rsem_isoktsp_count.txt"), "r")
    firstline, rest = f.readline(), f.read()
    t=open(os.path.join(args.outputfile, "rsem_isoktsp_count.txt"), "w")
    t.write(firstline[1:])
    t.write(rest)
    t.close()
    f.close()
