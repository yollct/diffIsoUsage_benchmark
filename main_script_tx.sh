...
parallel --will-cite -j $nCores "
  ...
  if [ $pairstate == "paired" ]; then
    ...
    ! test -d ${outputdir}/salmon_out/{} && mkdir -p ${outputdir}/salmon_out/{} || true # to allow mkdir to fail gracefully, we add '|| true'
    if ! test -f ${outputdir}/salmon_out/{}/quant.sf
      then salmon quant -i ${index}/salmon_index -p 8 -l SF -o ${outputdir}/salmon_out/{} --validateMappings -r ${readfilesdir}/{}.${fastaq} --seqBias --gcBias --dumpEq 2> ${outputdir}/salmon_out.stdout 
    fi
...
set +eu
conda deactivate
conda activate /nfs/scratch/chit/.conda/env/htseq
set -eu
...
if ! grep -q "nbsplice" ${outputdir}/results/salmon_res_tx_${groupl[0]}_${groupl[1]}.txt || ! grep -q "nbsplice" ${outputdir}/results/kal_res_tx_${groupl[0]}_${groupl[1]}.txt || ! grep -q "nbsplice" ${outputdir}/results/rsem_res_tx_${groupl[0]}_${groupl[1]}.txt; 
  ...
  apptainer run --mount type=bind,src=${outputdir},dst=/mnt \
                ...
                /nfs/proj/is_benchmark/runner/docker_nbsplice/nbsplice.sif
...
set +eu
conda activate /nfs/scratch/chit/.conda/env/saturn
set -eu
...
if ! grep -q "BANDITS" ${outputdir}/results/salmon_res_tx_${groupl[0]}_${groupl[1]}.txt; then
    ...
    for i in $samples;
    do  
      if ! test -f ${outputdir}/salmon_out/${i}/aux_info/eq_classes.txt; then
        if ! test -f ${outputdir}/salmon_out/${i}/aux_info/eq_classes.txt.gz; then
          echo "run salmon for bandits"
          if [ $pairstate == "paired" ]; then
            salmon quant -i ${index}/salmon_index -p 12 --validateMappings --seqBias --gcBias --dumpEq -l A -o ${outputdir}/salmon_out/${i} -1 ${readfilesdir}/${i}_1.${fastaq} -2 ${readfilesdir}/${i}_2.${fastaq} 2> ${outputdir}/salmon_out.stdout 
          else 
            salmon quant -i ${index}/salmon_index -p 8 -l SF -o ${outputdir}/salmon_out/${i} --validateMappings -r ${readfilesdir}/${i}.${fastaq} --seqBias --gcBias --dumpEq 2> ${outputdir}/salmon_out.stdout 
          fi
        fi
        pigz -d ${outputdir}/salmon_out/${i}/aux_info/eq_classes.txt.gz
      fi
    done
    ...
    Rscript ${path}/runner/bandits_salmon.R --gene_to_transcript /nfs/scratch/olga.tsoy/IS_paper/ens98_versions_human.txt --dir ${outputdir}/ --salmon salmon_out --ncores 10 --image salmon_test.RData --gene_results ${outputdir}/results/bandits_sal_gene.csv --tr_results ${outputdir}/results/bandits_sal_transcript.csv
fi
...
if ! grep -q "BANDITS" ${outputdir}/results/kal_res_tx_${groupl[0]}_${groupl[1]}.txt; then
    ...
    for i in $samples;
    do 
      if ! test -f ${outputdir}/kallisto_out/${i}/pseudoalignments.ec; then
        if [ $pairstate == "paired" ]; then
          kallisto pseudo -i /nfs/scratch/chit/ref/ens98_star_rsem/kallisto-index -o ${outputdir}/kallisto_out/${i} ${readfilesdir}/${i}_1.${fastaq} ${readfilesdir}/${i}_2.${fastaq} 
        else 
          kallisto pseudo -i ${index}/kallisto-index -o ${outputdir}/kallisto_out/${i} --single -t 8 -l 66 -s 35 ${readfilesdir}/${i}.${fastaq} 2> ${outputdir}/kallisto.stdout
        fi
      fi
    done
    ...
    Rscript ${path}/runner/bandits_kallisto.R --gene_to_transcript /nfs/scratch/olga.tsoy/IS_paper/ens98_versions_human.txt --dir ${outputdir}/ --kallisto kallisto_out --ncores 10 --image kallisto_test.RData --gene_results ${outputdir}/results/bandits_kal_gene.csv --tr_results ${outputdir}/results/bandits_kal_transcript.csv
fi


# echo "#########################"
# echo "Running bayesdrim from R"
# echo "#########################"
# if ! grep -q "BayesDRIMSeq" ${outputdir}/results/salmon_res_gene_${groupl[0]}_${groupl[1]}.txt; then
#     Rscript ${path}/runner/bayesdrimseq.R --input ${outputdir}/results/salmon_count.csv --dir ${outputdir}/ --tool salmon
# fi

# if ! grep -q "BayesDRIMSeq" ${outputdir}/results/kal_res_gene_${groupl[0]}_${groupl[1]}.txt; then
#     Rscript ${path}/runner/bayesdrimseq.R --input ${outputdir}/results/kal_count.csv --dir ${outputdir}/ --tool kal
# fi

# if ! grep -q "BayesDRIMSeq" ${outputdir}/results/rsem_res_gene_${groupl[0]}_${groupl[1]}.txt; then
#     Rscript ${path}/runner/bayesdrimseq.R --input ${outputdir}/results/rsem_count.csv --dir ${outputdir}/ --tool rsem
# fi
...
#   then
#   apptainer run --bind ${outputdir}:/MOUNT --bind /nfs/proj/is_benchmark/Rlib:/usr/local/lib/R/site-library /nfs/proj/is_benchmark/runner/sing_junctionseq/jcseq.sif 
