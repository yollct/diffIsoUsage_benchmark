...
path <- "/nfs/proj/is_benchmark/noise_fig"
            ...
            do.call(rbind, lapply(thisrep, function(rep){
                do.call(rbind, lapply(noises, function(noise){
                    ...
                    do.call(rbind, lapply(c("salmon", "kal", "rsem"), function(quant){
                        print(paste0(sprintf(outdir, seq,rep, R, noise), "/results/", sprintf("%s_res_gene_N_T.txt", quant)))
                        df_g <- read_delim(paste0(sprintf(outdir, seq,rep, R, noise), "/results/", sprintf("%s_res_gene_N_T.txt", quant)))
                        print( colnames(df_g))
...
overalldffdr_no_na <- overalldffdr[!is.na(overalldffdr$fdr), ]
...
for (rps in c("4 replicates", "8 replicates")) {
  for (spl in sort(unique(overalldffdr_no_na$quant_tool))) {
    data <- overalldffdr_no_na %>%
      ...
      select(noise, fdr, tool) %>%
      pivot_wider(names_from = tool, values_from = fdr, id_cols = noise, values_fn = mean) %>%
      as.data.frame()
    
    row.names(data) <- paste0("bg-", data$noise)
    print(data)
    data <- rbind(rep(1.5, ncol(data)), rep(0, ncol(data)), data)
...
colors <- pal_lancet("lanonc")(length(rownames(data)) - 2)
...
png(sprintf("%s/radarlegend.png", path), width=400, height=400, res=100)
plot.new()
# Add the legend
legend("center", # Position of the legend
       legend = sub("^_","",noises),  # List of names
       fill = colors_border,    # Colors corresponding to each name
       title = "Background", # Optional title # Filled circles
      pt.cex =5, # Point size
